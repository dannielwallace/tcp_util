...
func (this *TcpConn) read() []byte {
	...
	datalen := getUint(this.head, this.pack)
	if datalen <= this.pack {
		return nil
	}

	var buff = this.memPool.Alloc(this.padding + datalen - this.pack)
...
func (this *TcpConn) NewPackage(size int) *TcpOutput {
	...
	setUint(buff, this.pack, this.pack+size)
...
func (this *TcpConn) sendRaw(msg []byte) error {
	sendlen, err := this.conn.Write(msg)
	msg = msg[sendlen:]
	for err == nil && len(msg) > 0 {
		sendlen, err = this.conn.Write(msg)
		msg = msg[sendlen:]
